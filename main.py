...
# --- اصلاح: این دو تا دیکشنری/ست قبلاً هیچ‌وقت پاک نمی‌شدن و در اجرای طولانی‌مدت
# (چند هفته/ماه) کم‌کم حافظه اشغال می‌کردن. حالا هم قفل (Lock) دارن -- چون از دو
# ترد جدا (حلقه‌ی اصلی اسکن بازار + ترد گوش‌دادن به دکمه‌های تلگرام) بهشون دسترسی
# پیدا می‌شه -- و هم به‌صورت دوره‌ای پاکسازی می‌شن (نگاه کنید به prune_state_dicts).
...
CONSUMED_MESSAGES_MAX = 5000  # بالاتر از این تعداد، قدیمی‌ترین‌ها دور ریخته می‌شن


def prune_state_dicts():
    """
    پاکسازی دوره‌ای حافظه: ورودی‌های قدیمی LAST_ALERT_STATE (که دیگه تو محدوده‌ی
    کول‌داون نیستن) رو حذف می‌کنه، و اگه CONSUMED_MESSAGES خیلی بزرگ شد خالیش می‌کنه.
    این تابع باید هر چند چرخه یک‌بار (مثلاً در حلقه‌ی اصلی) صدا زده بشه.
    """
    now = time.time()
    with STATE_LOCK:
        stale_keys = [
            k for k, v in LAST_ALERT_STATE.items()
            if now - v["time"] > ALERT_COOLDOWN_SECONDS
        ]
        for k in stale_keys:
            del LAST_ALERT_STATE[k]

        if len(CONSUMED_MESSAGES) > CONSUMED_MESSAGES_MAX:
            CONSUMED_MESSAGES.clear()

        if stale_keys or len(CONSUMED_MESSAGES) == 0:
            log(f"🧹 پاکسازی حافظه: {len(stale_keys)} هشدار قدیمی حذف شد. "
                f"وضعیت فعلی: {len(LAST_ALERT_STATE)} هشدار فعال، {len(CONSUMED_MESSAGES)} پیام مصرف‌شده.")
...
def extract_binance_fill(order_result):
    """
    اصلاح: قبلاً بعد از سفارش MARKET، مقدار و قیمتِ واقعیِ پرشده خونده نمی‌شد و
    برنامه فقط از روی قیمت لحظه‌ای «تخمین» می‌زد -- در نوسان شدید یا Slippage،
    این تخمین می‌تونست با مقدار واقعی فرق کنه و حد سود/ضرر روی عدد اشتباه بسته بشه.
    این تابع مقدار و میانگین قیمتِ واقعیِ fill شده رو مستقیماً از پاسخ بایننس
    (فیلد fills) استخراج می‌کنه. اگه fills خالی/موجود نبود None برمی‌گردونه تا
    کد صدازننده به روش قدیمی (تخمین از قیمت لحظه‌ای) برگرده.
    """
...
def execute_buy_with_protection(exchange, symbol_or_base, sl_percent, limit_price=None):
    ...
    try:
        ...
        # اصلاح: برای سفارش MARKET بایننس، اول سعی می‌کنیم مقدار و قیمتِ واقعیِ
        # پرشده رو مستقیماً از پاسخ صرافی (فیلد fills) بخونیم -- دقیق‌تر از تخمین
        # زدن با قیمت لحظه‌ای، مخصوصاً وقتی بازار نوسان داره (Slippage).
        actual_fill = None
        if exchange == "binance" and not limit_price:
            actual_fill = extract_binance_fill(buy_result)

        if actual_fill:
            entry_price = actual_fill["avg_price"]
            raw_quantity = actual_fill["quantity"]
        else:
            # برای سفارش لیمیت، مبنای محاسبه‌ی حد سود/ضرر همون قیمتیه که سفارش روش
            # ثبت شده. برای نوبیتکس یا وقتی fills در دسترس نبود، از قیمت لحظه‌ای
            # به‌عنوان تخمین استفاده می‌کنیم (روش قبلی، به‌عنوان راه جایگزین).
        ...
        def _place_binance_protection():
            ...
            tp_res = binance_place_limit_order(symbol_or_base, "SELL", quantity, tp_price)
            sl_res = binance_place_stop_loss_order(symbol_or_base, quantity, sl_stop_price, sl_limit_price)
            return tp_res, sl_res

        def _place_nobitex_protection():
            quantity = round(raw_quantity, 6)
            tp_res = nobitex_place_limit_order(symbol_or_base, "sell", quantity, take_profit_price)
            sl_res = nobitex_place_stop_loss_order(symbol_or_base, quantity, stop_loss_price)
            return tp_res, sl_res

        place_protection = _place_binance_protection if exchange == "binance" else _place_nobitex_protection
        interpret_error = interpret_binance_error if exchange == "binance" else interpret_nobitex_error

        tp_result, sl_result = place_protection()
        tp_error = interpret_error(tp_result)
        sl_error = interpret_error(sl_result)

        # اصلاح: قبلاً اگه ثبت حد سود/ضرر شکست می‌خورد، پوزیشن بدون هیچ محافظتی
        # می‌موند و فقط یه پیام هشدار می‌رفت. حالا یک تلاش مجدد (retry) خودکار
        # انجام می‌شه؛ اگه بازم شکست خورد، هشدار با لحن فوری‌تر ارسال می‌شه تا
        # کاربر خودش دستی وارد عمل بشه.
        if tp_error or sl_error:
            log(f"⚠️ ثبت اولیه‌ی حد سود/ضرر برای {symbol_or_base} ناموفق بود، تلاش دوباره...")
            time.sleep(2)
            tp_result, sl_result = place_protection()
            tp_error = interpret_error(tp_result)
            sl_error = interpret_error(sl_result)
            if tp_error or sl_error:
                send_telegram_message(
                    f"🚨🚨 هشدار مهم: خرید {symbol_or_base} در {exchange} انجام شد ولی حد سود/ضرر "
                    f"حتی بعد از تلاش دوباره ثبت نشد! این پوزیشن الان بدون محافظت خودکاره -- "
                    f"لطفاً هرچه سریع‌تر دستی بررسی کنید.\n"
                    f"خطای حد سود: {tp_error or 'ندارد'}\nخطای حد ضرر: {sl_error or 'ندارد'}"
                )
        ...
        fill_note = " (قیمت/مقدار واقعیِ پرشده از صرافی)" if actual_fill else ""
        caption = (
            f"{mode_label}\n"
            f"صرافی: {exchange} | نماد: {symbol_or_base}\n"
            f"نوع سفارش خرید: {order_type_label}\n"
            f"قیمت خرید: {entry_price}{fill_note}\n"
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            log(f"خطا در پاکسازی دوره‌ای حافظه: {e}")
